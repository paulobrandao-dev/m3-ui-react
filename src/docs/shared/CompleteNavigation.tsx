...
import { Font, IconButton, NavLink } from '../../lib';
import { BASE_PATH } from '../constants';
import { Link } from '../router';

export default function CompleteNavigation() {
  const [current, setCurrent] = useState<string>(window.location.pathname);
  const [componentsIsHidden, hideComponents] = useState<boolean>(false);
  const [hooksIsHidden, hideHooks] = useState<boolean>(false);
  ...
  return (
    <>
      ...
      >
        Components
        <IconButton
          onClick={() => hideComponents(currentState => !currentState)}
        >
          <Icon
            symbol={
              componentsIsHidden ? 'expand_circle_down' : 'expand_circle_up'
            }
            size={20}
          />
        </IconButton>
          ...
          <NavLink
            ...
            icon={
              <Icon
                ...
                isFilled={
                  current === `${BASE_PATH}/components/canonical-layout`
                }
              />
            }
      ...
      >
        Hooks
        <IconButton onClick={() => hideHooks(currentState => !currentState)}>
          <Icon
            symbol={hooksIsHidden ? 'expand_circle_down' : 'expand_circle_up'}
            size={20}
          />
        </IconButton>
      </Font>
